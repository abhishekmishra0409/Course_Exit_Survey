...
const AllFeedback = () => {
  ...
  const [fillteredStudent, setFillteredStudent] = useState([]);
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://course-exit-survey.onrender.com/get-all-feedback"
        );
        setAllFeedback(response.data);

        // Extract unique options for dropdowns
        const uniqueCourses = [
          ...new Set(response.data.map((feedback) => feedback.selectedCourse)),
        ];
        const uniqueBranches = [
          ...new Set(response.data.map((feedback) => feedback.selectedBranch)),
        ];
        const uniqueSemesters = [
          ...new Set(
            response.data.map((feedback) => feedback.selectedSemester)
          ),
        ];
        const uniqueSessions = [
          ...new Set(response.data.map((feedback) => feedback.session)),
        ];
        ...
        const filteredStudents = response.data.filter(
          (feedback) =>
            (selectedCourse
              ...
              : true) &&
            (selectedBranch
              ? feedback.selectedBranch === selectedBranch
              : true) &&
            (selectedSemester
              ? feedback.selectedSemester === selectedSemester
              : true) &&
        ...
        setFillteredStudent(filteredStudents);
  ...
  const calculateSubjectQuestionAverage = (subject, questionIndex) => {
    const questionRatings = filteredFeedback.map((feedback) =>
      feedback.subjects[subject]
        ? feedback.subjects[subject][questionIndex]
        : null
    );
  ...
  const subjectColumns = Array.from(
    new Set(
      filteredFeedback.flatMap((feedback) => Object.keys(feedback.subjects))
    )
  );

  const overallAverages = subjectColumns.map((subject) => ({
    subject,
    averages: Array.from({ length: 9 }, (_, questionIndex) =>
  ...
  useEffect(() => {
    ...
    filteredFeedbackForSubject.forEach((feedback) => {
      if (feedback.subjects[selectedSubject]) {
        Object.entries(feedback.subjects[selectedSubject]).forEach(
          ([question, rating]) => {
    ...
    for (let i = 0; i <= questionCount; i++) {
      const questionRatings = filteredFeedbackForSubject.map((feedback) =>
        feedback.subjects[selectedSubject]
          ? feedback.subjects[selectedSubject][i.toString()]
          : null
      );
  ...
  const handleExportToExcel = () => {
    console.log(fillteredStudent);
    const worksheet = XLSX.utils.json_to_sheet(fillteredStudent);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Filtered Data");
    XLSX.writeFile(workbook, "filteredStudents.xlsx");
  };
                  ...
                  {selectedSubject && (
                    <div>
                      <table>
                        ...
                        <tbody>
                          {Object.entries(
                            subjectRatingsCount.questionCounts
                          ).map(([question, ratings]) => {
                      ...
                      <div className="bar-chart">
                        ...
                        <ReactApexChart
                          options={{
                            ...
                            xaxis: {
                              categories: selectedSubjectData.map(
                                (item) => item.question
                              ),
                            },
                            yaxis: {
